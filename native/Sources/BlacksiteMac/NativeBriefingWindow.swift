...
struct NativeBriefingDraft {
    ...
    var loadout: LoadoutDefinition { .init(camouflage: camouflage) }
}

@MainActor
final class NativeBriefingView: NSView {
    static let size = NSSize(width: 860, height: 550)
    let mapChoice = NSPopUpButton(), missionChoice = NSPopUpButton()
    let difficultyChoice = NSPopUpButton(), patternChoice = NSPopUpButton()
    ...
    init(draft: NativeBriefingDraft, maps: [MapDefinition] = PublishedMapRegistry.maps,
        ...
        popup(patternChoice, label: "Tarnmuster", titles: CamouflagePattern.allCases.map(NativeCamouflagePresentation.name),
              selected: CamouflagePattern.allCases.firstIndex(of: draft.camouflage) ?? 0, y: 187)
        rules.font = .systemFont(ofSize: 12); rules.frame = NSRect(x: 24, y: 240, width: 400, height: 76)
        inventory.font = .systemFont(ofSize: 12, weight: .medium); inventory.frame = NSRect(x: 24, y: 326, width: 400, height: 55)
        terrainAdvice.font = .systemFont(ofSize: 12); terrainAdvice.textColor = .secondaryLabelColor
        terrainAdvice.frame = NSRect(x: 24, y: 390, width: 400, height: 99)
        for label in [rules, inventory, terrainAdvice] { addSubview(label) }
        mapView.frame = NSRect(x: 448, y: 62, width: 388, height: 416); addSubview(mapView)
        for (index, button) in [cancelButton, applyButton, startButton].enumerated() {
            button.bezelStyle = .rounded; button.target = self
            button.frame = NSRect(x: 384 + index * 152, y: 504, width: 148, height: 30); addSubview(button)
        }
        ...
        let controls: [NSView] = [mapChoice, missionChoice, difficultyChoice, patternChoice, cancelButton, applyButton, startButton]
...
final class NativeBriefingPanel: NSPanel {
    ...
    init(draft: NativeBriefingDraft, interactionLabel: String) {
        briefingView = NativeBriefingView(draft: draft, interactionLabel: interactionLabel)
